...
商談ステータス別の件数、リードランク別の件数、担当ユーザー別の活動量、直近の活動履歴を 1 画面に集約。
`selectRaw` + `groupBy` による集計クエリで、営業活動の全体像をひと目で把握できます。

![ダッシュボード画面。商談ステータス別件数、リードランク別件数、担当ユーザー別活動件数、最近の活動一覧を表示](docs/images/dashboard.png)

### リード一覧（フィルタ検索 + CSV インポート / エクスポート）

会社名の部分一致に加え、ランク・状況・商談後ステータスを組み合わせた絞り込みが可能。
CSV の一括取り込みと書き出しにも対応し、スプレッドシート運用からの移行を想定しています。

![リード一覧画面。CSVインポート/エクスポートボタンと、会社名・ランク・状況・商談後ステータスによる検索フォーム、リードの一覧テーブルを表示](docs/images/lead-index.png)
